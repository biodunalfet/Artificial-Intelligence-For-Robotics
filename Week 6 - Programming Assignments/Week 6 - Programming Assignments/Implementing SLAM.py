...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    OmegaX = matrix([[]])
    OmegaX = OmegaX.expand(25, 25, [], [])

    OmegaY = matrix([[]])
    OmegaY = OmegaY.expand(25, 25, [], [])
    ...
    XiX.value[0][0] = world_size / 2.0
    XiY.value[0][0] = world_size / 2.0
    ...
    for i in range(len(data)):
        ...
        for j in range(len(Z)):
            ...
            #temp = matrix([[]])
            #temp = temp.expand(25, 25, [], [])
            L = 20 + Z[j][0]
            #print L
            
            OmegaX.value[i][i] += 1 / measurement_noise
            OmegaX.value[i][L] += -1 / measurement_noise
            OmegaX.value[L][i] += -1 / measurement_noise
            OmegaX.value[L][L] += 1 / measurement_noise

            OmegaY.value[i][i] += 1 / measurement_noise
            OmegaY.value[i][L] += -1 / measurement_noise
            OmegaY.value[L][i] += -1 / measurement_noise
            OmegaY.value[L][L] += 1 / measurement_noise

            #print str(20 + Z[i][0])
            XiX.value[i][0] += -Z[j][1] / measurement_noise
            XiX.value[L][0] += Z[j][1] / measurement_noise
            
            XiY.value[i][0] += -Z[j][2] / measurement_noise
            XiY.value[L][0] += Z[j][2] / measurement_noise

        #implement motions
        OmegaX.value[i][i] += 1 / motion_noise
        OmegaX.value[i][i + 1] += -1 / motion_noise
        OmegaX.value[i + 1][i] += -1 / motion_noise
        OmegaX.value[i + 1][i + 1] += 1 / motion_noise

        OmegaY.value[i][i] += 1 / motion_noise
        OmegaY.value[i][i + 1] += -1 / motion_noise
        OmegaY.value[i + 1][i] += -1 / motion_noise
        OmegaY.value[i + 1][i + 1] += 1 / motion_noise

        #print str(20 + Z[i][0])
        
        XiX.value[i][0] += -dx / motion_noise
        XiX.value[i + 1][0] += dx / motion_noise
        
        XiY.value[i][0] += -dy / motion_noise
        XiY.value[i + 1][0] += dy / motion_noise
    ...
    muX = OmegaX.inverse() * XiX
    muY = OmegaY.inverse() * XiY
    
    #mu = [[i, j] for i in muX for j in muY]
    
    u = []
    for i in range(len(OmegaX.value)):
        #print muX.value[i][0], muY.value[i][0]
